...
cursor.executemany('''
INSERT OR IGNORE INTO transacoes (id, cliente, data_transacao, produto, valor, quantidade)
VALUES (?, ?, ?, ?, ?, ?)
''', transacoes_data)
...
cursor.executemany('''
INSERT OR IGNORE INTO eventos_marketing (id, cliente, data_evento, canal, campanha, cliques, visualizacoes, custo)
VALUES (?, ?, ?, ?, ?, ?, ?, ?)
''', eventos_data)
...
cursor.execute('''
SELECT
    (SELECT SUM(cliques) FROM eventos_marketing),
    (SELECT SUM(visualizacoes) FROM eventos_marketing),
    (SELECT SUM(custo) FROM eventos_marketing),
    (SELECT COUNT(DISTINCT cliente) FROM transacoes),
    (SELECT SUM(valor * quantidade) FROM transacoes)
''')
totalCliques, totalVisu, totalCusto, clientesConvertidos, receita = cursor.fetchone()

CTR = totalCliques / totalVisu * 100
CPC = totalCusto / totalCliques
CPV = totalCusto / totalVisu
CAC = totalCusto / clientesConvertidos
ROI = (receita - totalCusto) / totalCusto * 100

print("Indicadores de Marketing:")
print(f"CTR (Taxa de Cliques): {CTR:.2f}%")
print(f"CPC (Custo por Clique): R$ {CPC:.2f}")
print(f"CPV (Custo por Visualização): R$ {CPV:.2f}")
print(f"CAC (Custo de Aquisição de Clientes): R$ {CAC:.2f}")
